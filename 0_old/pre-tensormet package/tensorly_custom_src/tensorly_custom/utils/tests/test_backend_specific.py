...
import tensorly_custom.utils.backend_specific
from tensorly_custom.utils.backend_specific import register_backend_specific


@pytest.fixture
def monkeypatch_available_backends(monkeypatch):
    monkeypatch.setattr(
        tensorly_custom.utils.backend_specific, "available_backend_names", ["backend"]
    )
...
def test_register_backend_specific_uses_correct_function(monkeypatch):
    ...
    monkeypatch.setattr(tensorly_custom.utils.backend_specific, "get_backend", get_backend)
    assert f() == 1

...
def initialize_tucker(
    ...
    svd : str, default is 'truncated_svd'
          function to use to compute the SVD, acceptable values in tensorly_custom.SVD_FUNS
...
def partial_tucker(
    ...
    svd : str, default is 'truncated_svd'
        ...
        acceptable values in tensorly_custom.tenalg.svd.SVD_FUNS
    tol : float, optional
...
def tucker(
    ...
    svd : str, default is 'truncated_svd'
        function to use to compute the SVD,
        acceptable values in tensorly_custom.SVD_FUNS
    tol : float, optional
          tolerance: the algorithm stops when the variation in
          the reconstruction error is less than the tolerance
    random_state : {None, int, np.random.RandomState}
...
def non_negative_tucker(
    ...
    epsilon = 10e-12
    step_time = print_elapsed_time(start_time)
    # Initialisation
    ...
    print_elapsed_time(start_time, "initialization")
    norm_tensor = tl.norm(tensor, 2)
    print("norm:", norm_tensor)
    step_time = print_elapsed_time(start_time, "normalization")
    rec_errors = []
    ...
    for iteration in range(n_iter_max):
        ...
        for mode in range(tl.ndim(tensor)):
            mode_time = time.time()
            print(f"\tstarting mode {mode} update")
            B = tucker_to_tensor((nn_core, nn_factors), skip_factor=mode)
            step_time = print_elapsed_time(step_time, "\t\ttucker to tensor")
            B = tl.transpose(unfold(B, mode))
            step_time = print_elapsed_time(step_time, "\t\tunfold and transpose")

            numerator = tl.dot(unfold(tensor, mode), B)
            step_time = print_elapsed_time(step_time, "\t\tnumerator computation (dot product)")
            numerator = tl.clip(numerator, a_min=epsilon, a_max=None)
            step_time = print_elapsed_time(step_time, "\t\tnumerator computation (clip)")
            denominator = tl.dot(nn_factors[mode], tl.dot(tl.transpose(B), B))
            step_time = print_elapsed_time(step_time, "\t\tdenominator computation (dot products)")
            denominator = tl.clip(denominator, a_min=epsilon, a_max=None)
            step_time = print_elapsed_time(step_time, "\t\tdenominator computation (clip)")
            nn_factors[mode] *= numerator / denominator
            step_time = print_elapsed_time(mode_time, f"\tmode {mode} update complete")

        step_time = time.time()
        print("starting core update")
        numerator = tucker_to_tensor((tensor, nn_factors), transpose_factors=True)
        numerator = tl.clip(numerator, a_min=epsilon, a_max=None)
        step_time = print_elapsed_time(step_time, "\tcore numerator computation")
        for i, f in enumerate(nn_factors):
            if i:
                denominator = mode_dot(denominator, tl.dot(tl.transpose(f), f), i)
            else:
                denominator = mode_dot(nn_core, tl.dot(tl.transpose(f), f), i)
            step_time = print_elapsed_time(step_time, f"\tcore denominator computation mode {i}")
        denominator = tl.clip(denominator, a_min=epsilon, a_max=None)
        nn_core *= numerator / denominator
        step_time = print_elapsed_time(step_time, "\tcore update complete")
        rec_error = (
            tl.norm(tensor - tucker_to_tensor((nn_core, nn_factors)), 2) / norm_tensor
        )
        rec_errors.append(rec_error)
        step_time = print_elapsed_time(step_time, "reconstruction error computation")
        ...
        iter_time_end = print_elapsed_time(iter_time, f"iteration {iteration} end")
        print("-----------------------------------------")

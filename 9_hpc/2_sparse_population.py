...
import pickle
import ast
from tensormet.utils import notify_discord, DATA_DIR
import argparse
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="A path to vector CSV file"
    )
...
def populate_tensors(path_to_vectors, top_ks, save=True, path_to_tensors=None):
    ...
    if not path_to_tensors:
        ...
        path_to_tensors =f"data/tensors/{dataset}/"
    ...
    with open(path_to_vectors, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            (v, s, o) = vector[:3]
            v = v or "~" # if still None elements persist, we again map to "~"
            s = s or "~"
            o = o or "~"
    ...
    results = {}
    for top_k in top_ks:
        to_compute = {"counting", "sii", "sc",
                      "siiSoftPlus", "siiShifted", "scSoftPlus", "scShifted",
                      "vocabularies"}
        artifact_paths = {element: path_to_tensors / f"populated/{element}_{top_k}.pt" for element in to_compute}
        artifact_paths["vocabularies"] = path_to_tensors / f"vocabularies/{top_k}.pkl"
        to_compute = {k for k in to_compute if not artifact_paths[k].exists()}
        ...
        # Iterate over 'vectors' directly.
        # We use tqdm here so you can see progress immediately.
        for v, s, o in tqdm(vectors, desc=f"Populating top_k={top_k}"):
            if v in v_set and s in s_set and o in o_set:
                triple_counts[(v, s, o)] += 1
        ...
        if len(indices) == 0:
            ...
            sii_softplus_tensor = torch.sparse_coo_tensor(indices_tensor,
                                                 torch.empty((0,), dtype=torch.float32),
                                                 size=(V, S, O))
            sc_softplus_tensor = torch.sparse_coo_tensor(indices_tensor,
                                                 torch.empty((0,), dtype=torch.float32),
                                                 size=(V, S, O))
            ...
            print("base tensors created")


            # we create normalized versions to ensure positivity without simple clipping
            # sii
            vals = sii_tensor.values()
            softplus_vals = torch.nn.functional.softplus(vals)
            sii_softplus_tensor = torch.sparse_coo_tensor(
                sii_tensor.indices(),
                softplus_vals,
            ...
            # sc
            vals = sc_tensor.values()
            softplus_vals = torch.nn.functional.softplus(vals)
            sc_softplus_tensor = torch.sparse_coo_tensor(
                sc_tensor.indices(),
                softplus_vals,
                size=sc_tensor.size()
            ).coalesce()

        print(f"created sparse tensors of shape {sc_softplus_tensor.shape} with {sc_softplus_tensor._nnz()} non-zero entries")
        print(f"Approximate size in memory: {sc_softplus_tensor._nnz() * 3 * 4 / (1024 ** 2):.2f} MB (indices + values) each")
        vocab = {
            "vocab_v": vocab_v,
            "vocab_s": vocab_s,
            "vocab_o": vocab_o,
        ...
        if save:
            ...
            torch.save(sc_softplus_tensor, f"{path_to_tensors}/populated/scSoftPlus_{top_k}.pt")
            torch.save(sii_softplus_tensor, f"{path_to_tensors}/populated/siiSoftPlus_{top_k}.pt")
            with open(f"{path_to_tensors}/vocabularies/{top_k}.pkl", "wb") as f:
                pickle.dump(vocab, f)
            print("Tensors and vocabularies saved.")
            print()
        else:
            results[top_k] = (sii_softplus_tensor, sc_softplus_tensor, vocab)
...
if __name__ == "__main__":
    ...
    goal_ks = args.top_ks if args.top_ks is not None else [2000, 3000, 4000, 5000, 6000]
    path_to_vectors = (
        args.vectors if args.vectors is not None else f"/readonly{DATA_DIR}/vectors/fineweb_english_vectors.csv"
    )
    path_to_tensors = (
        args.tensors if args.tensors is not None else DATA_DIR / "tensors" / "fineweb-en"
    )

    start_time = time.time()
    print("starting tensor population")
    populate_tensors(path_to_vectors=path_to_vectors,
                     top_ks=goal_ks,

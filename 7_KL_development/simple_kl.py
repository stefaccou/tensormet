...
from utils import DATA_DIR, select_gpu, tree_to_device, notify_discord, PROJECT_DATA_DIR
...
def non_negative_tucker(
    ...
    for iteration in range(n_iter_max):
        # print(f"\nCore starting iteration {iteration}: {core[0]}")
        # print(f"Factor 1 starting iteration {iteration}: {factors[0]}")
        # iter_time = time.time()
        for mode in modes:
            ...
            A = factors[mode]
            R = tl.dot(A, Z)
            X_R_frac = X / (R + epsilon)
            numerator = X_R_frac @ Z_T
            ...
            factors[mode] *= (numerator / (denominator + epsilon))**alpha
        ...
        if iteration > 1 and verbose:
            error = rec_errors[-1]
            print(
                f"{iteration}: reconstruction error={error}, variation={rec_errors[-2] - rec_errors[-1]}."
            )
            if rec_errors[-1] < tol:
                print("stopping here")
                break
...
for rank in ranks:
    for dim in dims:
        for method in methods:
            ...
            # Convert from CuPy → NumPy
            if hasattr(errors, "get"):  # cupy ndarray
                errors = errors.get()
            else:  # list of cupy scalars
                errors = np.array([float(e) for e in errors])
            # we plot the error graph and save it to PROJECT_DATA_DIR
            plt.figure()
            plt.plot(errors)
            save_path = os.path.join(PROJECT_DATA_DIR, f"{method}_{rank[0]}r_{iters}i_{alpha}alpha.png")

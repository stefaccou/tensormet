    ...
    for top_k in top_ks:
        ...
        if len(indices) == 0:
            ...
            sc_softplus_tensor = torch.sparse_coo_tensor(indices_tensor,
                                                 torch.empty((0,), dtype=torch.float32),
                                                 size=(V, S, O))
            sii_shifted_tensor = torch.sparse_coo_tensor(indices_tensor,
                                                 torch.empty((0,), dtype=torch.float32),
                                                 size=(V, S, O))
            sc_shifted_tensor = torch.sparse_coo_tensor(indices_tensor,
                                                 torch.empty((0,), dtype=torch.float32),
                                                 size=(V, S, O))
        else:
            ...
            sii_shifted_tensor = torch.sparse_coo_tensor(
                sii_tensor.indices(),
                norm_vals,
            ...
            sc_shifted_tensor = torch.sparse_coo_tensor(
                sc_tensor.indices(),
                norm_vals,
                size=sc_tensor.size()
            ).coalesce()
        ...
        if save:
            ...
            torch.save(sc_shifted_tensor, f"{path_to_tensors}/populated/scShifted_{top_k}.pt")
            torch.save(sii_shifted_tensor, f"{path_to_tensors}/populated/siiShifted_{top_k}.pt")
...
if __name__ == "__main__":
    ...
    goal_ks = [10000, 15000, 20000]
    paths_to_vectors = [DATA_DIR/"vectors/fineweb_english_vectors.csv"]
    path_to_tensors = DATA_DIR / "tensors" / "fineweb-en"
    device = select_gpu()
